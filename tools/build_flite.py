#!/usr/bin/env python3
"""Build and atomically stage the source-built Flite companion."""
...
FLITE_VERSION = "2.2"
FLITE_COMMIT = "e9e2e37c329dbe98bfeb27a1828ef9a71fa84f88"
SUPPORTED_TARGETS = {
    "aarch64-apple-darwin": ("macos-arm64", "omnivox-flite-helper"),
...
class StagingError(RuntimeError):
    """A Cargo output cannot form a verified Flite companion directory."""


def usage() -> str:
    return (
        "usage: python tools/build_flite.py [cargo build arguments]\n\n"
        "Prepares locked Flite v2.2 source, builds omnivox-flite-helper with "
        "locked Cargo dependencies, and stages a self-contained flite/ directory.\n\n"
        "examples:\n"
        "  python3 tools/build_flite.py --release\n"
        "  python3 tools/build_flite.py --release --target aarch64-unknown-linux-gnu"
    )
...
def prepare_source(repository: Path) -> tuple[Path, dict[str, object]]:
    command = [sys.executable, str(repository / "tools/prepare_flite_inputs.py")]
    if configured := os.environ.get("OMNIVOX_FLITE_INPUTS_DIR"):
    ...
    output = (
        ...
        if configured
        else repository / "target/flite-inputs" / FLITE_VERSION
    )
    marker = json.loads((output / "PREPARED.json").read_text(encoding="utf-8"))
    source = Path(str(marker["source_path"]))
    if not source.is_dir():
        raise StagingError(f"prepared Flite source is missing: {source}")
...
def build(arguments: list[str], source: Path) -> Path:
    ...
    for argument in arguments:
        if argument == "-p" or argument.startswith(forbidden):
            raise StagingError(
                "tools/build_flite.py owns Cargo's package and message-format selection"
            )
    command = [
        "cargo",
        "build",
    ...
    environment["OMNIVOX_FLITE_SOURCE_DIR"] = str(source)
    print("+ " + " ".join(command), file=sys.stderr)
    ...
    for line in process.stdout:
        ...
        if (
            ...
            and target.get("name") == "omnivox-flite-helper"
    ...
    if len(executables) != 1:
        ...
        raise StagingError(
            f"Cargo reported {len(executables)} Flite helper executables; "
            f"refusing to guess:\n  {rendered}"
        )
...
def companion_readme(target: str) -> str:
    return f"""# Omnivox Flite companion

This `{target}` companion contains the Omnivox {FLITE_VERSION} helper and
Flite v{FLITE_VERSION}, statically built from commit `{FLITE_COMMIT}`. Its sole
compiled-in voice is `cmu_us_slt`.

Place this top-level `flite/` directory beside `omnivox` or `omnivox.exe`.
Optional local `.flitevox` files can be supplied through the platform path list
in `OMNIVOX_FLITE_VOICES`; no voices are downloaded at run time.

`LICENSE` covers Omnivox-authored code. `third-party-licenses/Flite-COPYING.txt`
is Flite's full upstream licence. `SOURCE-PROVENANCE.json` records the exact
source input used for this executable.
"""
...
def stage(
    ...
    if target not in SUPPORTED_TARGETS:
        raise StagingError(f"unsupported Flite companion target: {target}")
    ...
    try:
        ...
        shutil.copy2(
            repository / "omnivox-flite-sys/source-inputs.json",
            temporary / "source-inputs.json",
        )
        notices = temporary / "third-party-licenses"
        notices.mkdir()
        shutil.copy2(source / "COPYING", notices / "Flite-COPYING.txt")
        ...
        provenance = {
            "schema_version": 1,
            "artifact": f"omnivox-flite-companion-{suffix}",
            "target": target,
            "compiled_voice": "cmu_us_slt",
            "external_voice_files_included": False,
            "flite": {
                "version": FLITE_VERSION,
                "commit": FLITE_COMMIT,
        ...
        if destination.exists():
            if not destination.is_dir():
                raise StagingError(
                    f"Flite companion destination is not a directory: {destination}"
                )
    ...
    print(f"Staged Flite companion in {destination}", file=sys.stderr)
    return destination
