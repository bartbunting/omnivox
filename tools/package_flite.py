#!/usr/bin/env python3
"""Create a deterministic Flite companion release archive."""
...
EXPECTED_NOTICES = {"Flite-COPYING.txt", "omnivox-Cargo.lock"}


class PackagingError(RuntimeError):
    """The staged Flite companion cannot form a release archive."""
...
def parse_arguments(repository: Path, target: str) -> argparse.Namespace:
    ...
    parser.add_argument("--staged", type=Path, default=release / "flite")
    parser.add_argument(
        "--output",
        type=Path,
...
def validate_stage(directory: Path, repository: Path, target: str) -> None:
    ...
    require(directory.is_dir(), f"staged Flite directory is missing: {directory}")
    require(
        {path.name for path in directory.iterdir()} == EXPECTED_ROOT | {helper_name},
        "staged Flite root entries are incomplete or unexpected",
    )
    notices = directory / "third-party-licenses"
    require(
        {path.name for path in notices.iterdir()} == EXPECTED_NOTICES,
        "staged Flite notice set is incomplete or unexpected",
    )
    for path in directory.rglob("*"):
        require(not path.is_symlink(), f"staged Flite symlink is not allowed: {path}")
    ...
    require(expected == actual, "staged Flite SHA256SUMS does not match every file")
    ...
    require(
        ...
        "wrong Flite artifact provenance",
    )
    require(provenance.get("target") == target, "wrong Flite target provenance")
    require(provenance.get("compiled_voice") == "cmu_us_slt", "wrong bundled voice")
    require(
        provenance.get("external_voice_files_included") is False,
        "external voice exclusion is not recorded",
    )
    flite = provenance.get("flite")
    require(isinstance(flite, dict), "Flite source provenance is missing")
    require(flite.get("version") == FLITE_VERSION, "wrong Flite source version")
    require(flite.get("commit") == FLITE_COMMIT, "wrong Flite source commit")
    ...
    require(
        ...
        "refusing to package a Flite companion staged from a dirty worktree",
    )
    require(
    ...
    require(
        ...
        "staged Flite companion does not match the current source commit",
    )
        ...
        with temporary.open("wb") as raw:
            with gzip.GzipFile(filename="", mode="wb", fileobj=raw, mtime=timestamp) as compressed:
                with tarfile.open(fileobj=compressed, mode="w", format=tarfile.PAX_FORMAT) as archive:
                    archive.addfile(tar_info("flite", 0o755, timestamp, True))
                    for path in sorted(source.rglob("*")):
                        name = f"flite/{path.relative_to(source).as_posix()}"
                        if path.is_dir():
...
def write_zip(source: Path, destination: Path, timestamp: int) -> None:
    ...
    try:
        with zipfile.ZipFile(temporary, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
            archive.writestr(zip_info("flite", 0o755, timestamp, True), b"")
            for path in sorted(source.rglob("*")):
                name = f"flite/{path.relative_to(source).as_posix()}"
                if path.is_dir():
