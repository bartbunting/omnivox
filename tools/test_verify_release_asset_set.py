...
class ReleaseAssetSetTests(unittest.TestCase):
    def test_expected_set_has_26_archives_and_one_manifest(self) -> None:
        ...
        self.assertIn("omnivox-1.7.0-rutts-source.tar.gz", names)
        self.assertIn("omnivox-1.7.0-piper-macos-x64.tar.gz", names)
        self.assertIn("omnivox-1.7.0-tgspeechbox-windows-x64.zip", names)
        self.assertIn("omnivox-1.7.0-tgspeechbox-source.tar.gz", names)
        self.assertNotIn("omnivox-1.7.0-piper-windows-arm64.zip", names)

    def test_names_gate_rejects_stale_and_missing_assets(self) -> None:
        names = sorted(assets.expected_asset_names("1.7.0"))
        names.remove("omnivox-1.7.0-flite-linux-x64.tar.gz")
        names.append("omnivox-1.6.4-flite-linux-x64.tar.gz")
    ...
    def test_directory_gate_requires_exact_checksums(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            assets.verify_directory(directory, "1.7.0")
