#!/usr/bin/env python3
"""Verify the deterministic Flite source and build-integration archive."""
...
class SourceVerificationError(common.VerificationError):
    """The Flite source artifact violated its release contract."""
...
def verify(repository: Path, arguments: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="Omnivox Flite source verification ") as temporary:
        ...
        require(provenance.get("artifact") == "omnivox-flite-source", "wrong artifact provenance")
        require(provenance.get("flite_version") == FLITE_VERSION, "wrong Flite version")
        require(provenance.get("flite_commit") == FLITE_COMMIT, "wrong Flite commit")
        ...
        lock_path = root / "omnivox/omnivox-flite-sys/source-inputs.json"
        lock = json.loads(lock_path.read_text(encoding="utf-8"))
        require(sha256_file(lock_path) == provenance.get("source_input_lock_sha256"), "source lock digest mismatch")
        input_path = root / str(provenance.get("input_archive", ""))
        require(input_path.is_file(), "locked Flite input archive is missing")
        require(sha256_file(input_path) == lock.get("sha256"), "Flite input archive checksum mismatch")
        require(provenance.get("source_tree_sha256") == lock.get("source_tree_sha256"), "Flite tree digest mismatch")
        ...
        prepare = root / "omnivox/tools/prepare_flite_inputs.py"
        subprocess.run(
