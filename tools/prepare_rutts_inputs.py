#!/usr/bin/env python3
"""Download, verify, and safely extract the locked RuTTS source input."""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare the checksum-locked RuTTS v6.3.3 source tree."
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="prepared input directory (default: target/rutts-inputs/6.3.3)",
    )
    parser.add_argument(
        "--check",
        action="store_true",
...
def verify_source(source: Path, expected_digest: str) -> str:
    require(source.is_dir(), f"prepared RuTTS source is missing: {source}")
    for relative in REQUIRED_PATHS:
        require((source / relative).is_file(), f"RuTTS source is missing {relative}")
    actual = tree_digest(source)
    require(
        actual == expected_digest,
        f"prepared RuTTS tree checksum mismatch: found {actual}, "
...
def prepare(arguments: argparse.Namespace) -> Path:
    ...
    output = (
        ...
        if arguments.output
        else repository / "target/rutts-inputs" / str(lock["rutts_version"])
    )
    ...
    if archive.is_file():
        ...
        require(
            ...
            f"cached RuTTS archive checksum mismatch: found {actual_archive}, "
            f"expected {expected_archive}",
        )
    elif arguments.check:
        raise PreparationError(f"cached RuTTS archive is missing: {archive}")
    else:
        print("Downloading locked RuTTS v6.3.3 source", file=sys.stderr)
    ...
    try:
        ...
        temporary = Path(tempfile.mkdtemp(prefix=".rutts.tmp-", dir=output))
        try:
    ...
    marker = {
        ...
        "rutts_version": str(lock["rutts_version"]),
    }
    if arguments.check:
    ...
    print(f"Prepared and verified RuTTS inputs in {output}", file=sys.stderr)
    return output
