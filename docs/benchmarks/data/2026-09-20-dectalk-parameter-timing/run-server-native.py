...
parser.add_argument('output');parser.add_argument('--target',action='append',required=True,help='name=program');parser.add_argument('--iterations',type=int,default=10);parser.add_argument('--engines',default='eloquence,espeak,dectalk');parser.add_argument('--repeats',type=int,default=1)
...
targets=[]
for arg in a.target:
 name,program=arg.split('=',1);t=copy.deepcopy(template);t.update(id=name,program=program,native=False,ui=False);t['environment']['OMNIVOX_PROGRAM']=program;targets.append(t)
...
for repeat in range(a.repeats):
 ...
 for t,r in jobs:
  ...
  job={'audio_output':'null','cases':['line'],'concurrent':False,'flavour':'native','iterations':a.iterations,'mode':'warm','omnivox_root':plan['omnivox_root'],'rate':225,'replacement_burst':5,'route':r,'server':t['server'],'timeout':30,'warmups':3}
