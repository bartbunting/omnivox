...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let root = std::env::current_exe()?
        ...
        .to_path_buf();
    let library = match library_path(std::env::args_os().skip(1))? {
        None => None,
        Some((path, expected)) => Some(omnivox_tts::voice_library::RuntimeLibrary::read_expected(
            std::fs::File::open(path)?,
            if cfg!(windows) {
                omnivox_tts::voice_library::HostPlatform::Windows
            } else {
                omnivox_tts::voice_library::HostPlatform::Posix
            },
            expected.as_deref(),
        )?),
    };
    let engine = Arc::new(engine::MbrolaEngine::new(root, library.as_ref()));
...
fn library_path(
    ...
    if flag != "--voice-library" {
        return Err(format!("unknown MBROLA helper argument: {flag:?}"));
    }
    let path = arguments
        .next()
    ...
    if arguments.next().is_some() {
        return Err("unexpected MBROLA helper argument".to_owned());
    }
