...
// preset to answer a query. Verified application/readback stays on the DECtalk owner.
internal sealed class OmnivoxDectalkParameterService
{
    private readonly OmnivoxDectalkAdapter engine;
    ...
    internal OmnivoxDectalkParameterService(OmnivoxDectalkAdapter engine,
        string dllPath, bool bindingsAvailable)
    {
        ...
        for (int i = 0; i < OmnivoxDectalkParameters.Count; i++)
            rows.Add(OmnivoxDectalkParameters.Descriptor(i));
        descriptors = rows.ToArray();
        mappings = new object[] {
            Mapping(new string[] { "average_pitch" }, "ap"),
            Mapping(new string[] { "pitch_range" }, "as", "pr"),
            Mapping(new string[] { "stress" }, "bf", "qu", "hr", "sr"),
            Mapping(new string[] { "richness" }, "sm", "ri")
        };
        string content = OmnivoxParameterWire.Canonical(OmnivoxParameterWire.Map(
            "schema_id", OmnivoxDectalkParameters.SchemaId, "profile_id", OmnivoxDectalkParameters.ProfileId,
        ...
        identity = OmnivoxParameterWire.Map("schema_id", OmnivoxDectalkParameters.SchemaId,
            "profile_id", OmnivoxDectalkParameters.ProfileId, "catalogue_revision", revision,
    ...
    private string Qualification(out bool busy)
    {
        ...
        {
            busy = false;
            if (!bindingsAvailable) return "DECtalk speaker parameter API is unavailable";
            if (!qualificationStarted)
            {
                ...
                ThreadPool.QueueUserWorkItem(delegate(object unused) {
                    string error = null;
                    try { OmnivoxDectalkParameters.RequireQualifiedRuntime(engine.Version, dllPath); }
                    catch (Exception) { error = "This DECtalk runtime has no qualified native parameter profile"; }
                    lock (gate) { qualificationError = error; qualificationFinished = true; }
                });
            }
            if (!qualificationFinished)
            {
                if (qualificationTime.ElapsedMilliseconds >= 10000) return "DECtalk parameter qualification timed out";
    ...
    internal Dictionary<string, object> Catalogue(IDictionary<string, object> query)
    {
        ...
        // Twenty-eight descriptors fit one page. No continuation cursor is ever issued.
    ...
    {
        internal OmnivoxDectalkParameterService Service;
        internal OmnivoxParameterSettings Settings;
        internal OmnivoxDectalkParameters Edits;
            ...
            Func<bool> cancelled, IOmnivoxCaptureSink sink, Action<Dictionary<string, object>> applied)
        {
            ...
            return Service.engine.SynthesizeWithParameters(text, Settings.Voice, Settings.Rate, Settings.Pitch,
                ...
                Values, Context, delegate(int[] actual) {
                    if (cancelled()) throw new OperationCanceledException();
                    string id = "dt-" + Guid.NewGuid().ToString("N");
    ...
    private Plan Parse(OmnivoxParameterSettings settings, object parameters)
    {
        ...
        if (parameters != null)
        {
            ...
            bool ours = engineId == engine.EngineId && schema == OmnivoxDectalkParameters.SchemaId;
        ...
        try { plan.Edits = new OmnivoxDectalkParameters(plan.Values, plan.Context); }
        catch (ArgumentException) { OmnivoxParameterWire.Invalid("DECtalk parameter ID, value or contextual dimension"); }
        if (mismatch != null)
        {
            plan.UnavailableReason = mismatch;
            return plan;
        }
        string error = Qualification(out plan.Busy);
        plan.UnavailableReason = error ?? (plan.Busy ? "DECtalk parameter qualification is busy" : null);
        return plan;
    }
    ...
    private Dictionary<string, object> Explanation(Plan plan, int[] actual, string id)
    {
        ...
        int?[] common = OmnivoxDectalkAdapter.MapCommonParameters(s.Voice, s.Pitch,
            s.PitchRange, s.Stress, s.Richness);
