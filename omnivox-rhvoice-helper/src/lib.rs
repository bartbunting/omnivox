...
use omnivox_tts::{
    ...
    VoiceInfo, VoiceQuality, STANDARD_SAMPLE_RATE,
};
use thiserror::Error;
...
const RHVOICE_MESSAGE_TEXT: c_int = 0;
const RHVOICE_MESSAGE_SSML: c_int = 1;
...
struct RhVoiceInput {
    text: CString,
    message_type: c_int,
    /// For generated SSML, map each generated byte boundary back to the
    /// corresponding UTF-8 byte boundary in the caller's plain text.
...
impl RhVoiceInput {
    fn for_request(request: &SynthesisRequest) -> Result<Self, TtsError> {
        ...
        Ok(Self {
            text: CString::new(generated).map_err(|_| {
                TtsError::InvalidParameter("generated RHVoice SSML contains a null byte".to_owned())
            })?,
    ...
    fn source_range(&self, plain_text: &str, start: u32, length: u32) -> Option<(u32, u32)> {
        if let Some(boundaries) = &self.source_boundaries {
            let start = start as usize;
            let end = start.checked_add(length as usize)?;
            let source_start = *boundaries.get(start)?;
            let source_end = *boundaries.get(end)?;
            return Some((source_start, source_end.saturating_sub(source_start)));
        }
        ...
        .then_some((start as u32, length))
    }
}

fn append_rhvoice_anchor(
...
fn append_generated_text(
    ...
    source_boundaries.extend((0..byte_count).map(|index| {
...
impl TtsEngine for RhVoiceTtsEngine {
    ...
    fn synthesize(&self, request: &SynthesisRequest) -> Result<SynthesisResult, TtsError> {
        ...
        if request.text.is_empty() {
            ...
            result.anchors = exact_empty_anchors(request);
            return Ok(result);
        }

        let input = RhVoiceInput::for_request(request)?;
        let text_length = c_uint::try_from(input.text.as_bytes().len())
        ...
        let canonical_frame_count = audio.frame_count() as u64;
        ...
        let anchors = anchors_from_native(
            &request.anchors,
            &capture.anchors,
            sample_rate,
            capture.samples.len() as u64,
    ...
    fn synthesize_stream(
        ...
        if request.text.is_empty() {
            let anchors = exact_empty_anchors(request);
            if !anchors.is_empty() {
...
struct StreamingCapture<'a> {
    ...
    pending_anchors: Vec<NativeAnchor>,
    reported_anchors: HashSet<usize>,
    failure: Option<TtsError>,
}
...
impl<'a> SynthesisCapture<'a> {
    fn buffered(
        ...
        Self::Buffered(BufferedCapture {
            ...
            reported_anchors: HashSet::with_capacity(expected_anchor_count),
            cancellation,
    ...
    fn streaming(
        ...
        Self::Streaming(StreamingCapture {
            ...
            reported_anchors: HashSet::with_capacity(anchors.len()),
            failure: None,
        })
    }
    ...
    fn validate_anchors(&self) -> Result<(), TtsError> {
        let (reported, expected) = match self {
            Self::Buffered(capture) => (
                capture.reported_anchors.len(),
                capture.expected_anchor_count,
            ),
            Self::Streaming(capture) => (capture.reported_anchors.len(), capture.anchors.len()),
        };
        if reported != expected {
            return Err(synthesis_failure(format!(
                "RHVoice reported {reported} of {expected} requested anchors"
            )));
        }
        Ok(())
    }
    ...
    fn record_anchor(&mut self, index: usize) -> Result<c_int, TtsError> {
        match self {
            Self::Buffered(capture) => {
                if index >= capture.expected_anchor_count {
                    return Err(synthesis_failure(
                        "RHVoice returned an unknown requested anchor",
                    ));
                }
                if !capture.reported_anchors.insert(index) {
                    return Err(synthesis_failure(
                        "RHVoice returned a requested anchor more than once",
                    ));
                }
                capture.anchors.push(NativeAnchor {
                    index,
                    native_frame: capture.samples.len() as u64,
                });
            }
            Self::Streaming(capture) => capture.record_anchor(index)?,
        }
        Ok(i32::from(self.should_continue()))
    }
}
...
impl StreamingCapture<'_> {
    fn set_sample_rate(&mut self, sample_rate: u32) -> Result<(), TtsError> {
        ...
        if self.canonicalizer.is_none() {
            ...
            self.emit_native_anchors(pending)?;
        }
        Ok(())
    }
    ...
    fn record_anchor(&mut self, index: usize) -> Result<(), TtsError> {
        ...
        self.emit_native_anchors(vec![anchor])
    }
        ...
        let markers = markers
            ...
            .filter_map(|marker| {
                let (text_start, text_length) =
                    self.input
                        .source_range(self.text, marker.text_start, marker.text_length)?;
    ...
    fn emit_native_anchors(&mut self, anchors: Vec<NativeAnchor>) -> Result<(), TtsError> {
        let canonicalizer = self
            .canonicalizer
            .as_ref()
            .expect("RHVoice anchor emission requires a sample rate");
        let anchors = anchors
            .into_iter()
            .filter_map(|anchor| {
                let requested = self.anchors.get(anchor.index)?;
                let frame_offset = canonicalizer
                    .canonical_frame_offset(anchor.native_frame)
                    .ok()?;
                Some(ResolvedAnchor {
            ...
            .collect::<Vec<_>>();
        if !anchors.is_empty() {
            self.sink.markers(Vec::new(), anchors)?;
        }
        Ok(())
    }
...
fn native_callbacks() -> NativeCallbacks {
    NativeCallbacks {
        ...
        process_mark: Some(process_mark_callback),
...
unsafe extern "C" fn process_mark_callback(name: *const c_char, user_data: *mut c_void) -> c_int {
    with_capture(user_data, |capture| {
        if !capture.should_continue() {
            return Ok(0);
        }
        if name.is_null() {
            return Err(synthesis_failure("RHVoice returned a mark without a name"));
        }
        let name = unsafe { CStr::from_ptr(name) }
            .to_str()
            .map_err(|_| synthesis_failure("RHVoice returned a non-UTF-8 mark name"))?;
        let Some(index) = name
            .strip_prefix("ovx")
            .and_then(|value| value.parse::<usize>().ok())
        else {
            return Err(synthesis_failure("RHVoice returned an unknown mark name"));
        };
        capture.record_anchor(index)
    })
}
...
fn markers_from_native(
    ...
    let mut result = markers
        ...
        .filter_map(|marker| {
            let (text_start, text_length) =
                input.source_range(text, marker.text_start, marker.text_length)?;
            Some(SynthesisMarker {
                kind: marker.kind,
                frame_offset: scale_frame(marker.native_frame.min(native_frame_count), sample_rate)
                    .min(canonical_frame_count),
                text_start: Some(text_start),
                text_length: Some(text_length),
                value: None,
            })
...
fn anchors_from_native(
    ...
    anchors
        .iter()
        .filter_map(|anchor| {
            let requested = requested.get(anchor.index)?;
            Some(ResolvedAnchor {
                id: requested.id.clone(),
                frame_offset: Some(
                    scale_frame(anchor.native_frame.min(native_frame_count), sample_rate)
                        .min(canonical_frame_count),
                ),
                resolution: AnchorResolution::Exact,
            })
...
fn exact_empty_anchors(request: &SynthesisRequest) -> Vec<ResolvedAnchor> {
    request
        .anchors
        .iter()
        .map(|anchor| ResolvedAnchor {
...
mod tests {
    ...
    fn native_markers_are_validated_and_rescaled() {
        let request = SynthesisRequest::new("héllo world", omnivox_tts::TtsSettings::default());
        let input = RhVoiceInput::for_request(&request).unwrap();
        let markers = markers_from_native(
            "héllo world",
            &input,
            &[
    ...
    fn streaming_callbacks_emit_markers_before_progressive_audio() {
        ...
        let request = SynthesisRequest::new("hello world", omnivox_tts::TtsSettings::default());
        let input = RhVoiceInput::for_request(&request).unwrap();
        let mut capture = SynthesisCapture::streaming(
            &mut sink,
            &request.text,
            &input,
            &request.anchors,
    ...
    #[test]
    fn native_rhvoice_marks_stream_as_exact_anchors() {
        let engine = RhVoiceTtsEngine::from_environment();
        let descriptor = engine.descriptor();
        if !descriptor.can_synthesize() {
            return;
        }
        let settings = omnivox_tts::TtsSettings {
            voice: descriptor.default_voice_id.unwrap(),
            ..omnivox_tts::TtsSettings::default()
        };
        let request = SynthesisRequest::new("héllo & world", settings)
            .with_anchors(vec![
                RequestedAnchor::new("start", 0, omnivox_tts::AnchorAffinity::Before),
                RequestedAnchor::new("middle", 9, omnivox_tts::AnchorAffinity::After),
                RequestedAnchor::new("end", 14, omnivox_tts::AnchorAffinity::After),
            ])
            .unwrap();
        let buffered = engine
            .synthesize(&request)
            .expect("buffered anchored RHVoice synthesis failed");
        buffered.validate(&request).unwrap();
        assert_eq!(buffered.anchors.len(), 3);
        assert!(buffered
            .anchors
            .iter()
        ...
        let completion = engine
            ...
            .expect("anchored RHVoice synthesis failed");
        ...
        assert!(word_ranges.contains(&(Some(9), Some(5))));
    }
